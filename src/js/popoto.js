...
// QUERY ------------------------------------------------------------------------------------------------------------
    ...
    popoto.query.generateTaxonomyCountQuery = function (label) {
        ...
        if (constraintAttr === popoto.query.NEO4J_INTERNAL_ID) {
            return "match (n:`" + label + "`) return count(DISTINCT ID(n)) as count"
        } else {
            return "match (n:`" + label + "`) return count(DISTINCT n." + constraintAttr + ") as count"
        }
    };
        ...
        if (popoto.result.hasChanged) {
            ...
            if (popoto.result.resultCountListeners.length > 0) {
                ...
                popoto.rest.post(
                    ...
                    .done(function (data) {
                        var count = data.results[0].data[0].row[0];
